...
def generateData(size=5000, verbose=False):
    ...
    W2 = np.random.normal(0, 2, size)
    
    A = np.random.binomial(1, expit(0.52 + 2*W1 + 3*W3), size)
    if verbose:
       print("proportion of A=1:", np.bincount(A)[1]/size)

    Y = np.random.binomial(1, expit(3*A + 2*W1 + 2*W2), size)
    if verbose:
    ...
    params = [1.5, 1.5, -2]
    pRY_Y_0 = expit(params[0]*W2 + params[1]*I)
    # pRY_Y_0 = expit(0)
...
if __name__ == "__main__":
    ...
    if result is None:
        ...
        shadowRecoveryIgnoreMissingness = ShadowRecovery("A", "Y", "R_Y", ["W1"], subset_data, ignoreMissingness=True)
        ...
        print("missingness bias causal effect", backdoor_adjustment_binary("Y", "A", ["W1"], subset_data))
        print("confidence intervals", compute_confidence_intervals("Y", "A", ["W1"], subset_data, "backdoor_binary"))
        print()

        shadowRecovery_backdoorBias = ShadowRecovery("A", "Y", "R_Y", ["W2"], partial_data)
