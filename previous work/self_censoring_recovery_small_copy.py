...
def generateData(size=5000, verbose=False):
    ...
    W2 = np.random.normal(0, 1, size) + 1.4*W1 + 0.8*W3
    ...
    params = [0.33, 4]
    R_A_temp = expit(params[0]) / ( expit(params[0]) + params[1]**A * (1-expit(params[0])) )
    ...
    full_data = pd.DataFrame({"A": A, "Y": Y, "W1": W1, "W2": W2, "W3": W3, "R_A": R_A, "R_Y": R_Y, "pRY_1": pRY_1})
    ...
    Y = full_data["Y"].copy()
    Y[full_data["R_Y"] == 0] = -1
    partial_data = pd.DataFrame({"A": A, "Y": Y, "W1": W1, "W2": W2, "W3": W3, "R_A": R_A, "R_Y": R_Y, "pRY_1": pRY_1})
...
def shadowIpwFunctional_Y(params, W_1, W_2, W_3, R_Y, Y, data):
    ...
    # output3 = np.average( ((data[W_1]**2)*data[R_Y]) / pRY_1 - (data[W_1]**2) )
    # output4 = np.average( ((data[W_2]**2)*data[R_Y]) / pRY_1 - (data[W_2]**2) )

    output1 = np.average( ((data[W_2])*data[R_Y]) / pRY_1 - (data[W_2]) )
    output2 = np.average( ((data[W_3])*data[R_Y]) / pRY_1 - (data[W_3]) )
    output3 = np.average( (data[R_Y]) / pRY_1 - (1) )
    output4 = np.average( ((data[W_1])*data[R_Y]) / pRY_1 - (data[W_1]**2) )

    return [output1, output2, output3, output4]

def rootsPrediction_Y(roots, W_2, W_3, Y, data):
    """
    Use the values of alpha and gamma contained in roots to make predictions on the value
    Y based on W2.
    """
    pRY_1_Y_0 = expit(roots[0]*data[W_2]+roots[1]*data[W_3])
    predictions = pRY_1_Y_0 / (pRY_1_Y_0 + (np.exp(roots[2]*data[Y])) * (1-pRY_1_Y_0))
...
if __name__ == "__main__":
    ...
    roots_RY = optimize.root(shadowIpwFunctional_Y, [0.0, 0.0, 0.0, 0.1], args=("W1", "W2", "W3", "R_Y", "Y", partial_data), method='hybr')
    print(roots_RY.x)
    ...
    # print(rootsPrediction_Y(roots_RY.x, "W2", "W3", "Y", partial_data_Y))

    # print(np.average(np.abs(partial_data_Y["pRY_1"] - rootsPrediction_Y(roots_RY.x, "W2", "W3", "Y", partial_data_Y))))
